...
class SFV0AgentConfig:
    # @package agent
    _target_: str = "url_benchmark.agent.sf_v0.SFV0Agent"
    name: str = "sf"
...
cs.store(group="agent", name="sf_v0", node=SFV0AgentConfig)
...
class SFV0Agent:
    ...
    def update_sf(
        ...
        with torch.no_grad():
            ...
            target_phi_Q = torch.einsum('sd, sd -> s', target_phi, z)
            next_Q = torch.min(next_Q1, next_Q2)
            target_Q = target_phi_Q + discount * next_Q
