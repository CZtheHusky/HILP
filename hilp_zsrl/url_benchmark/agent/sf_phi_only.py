...
class SFAgent:
    ...
    def __init__(self, **kwargs: tp.Any):
        ...
        self.feature_learner_bk = learner(self.obs_dim, self.action_dim, cfg.z_dim, cfg.phi_hidden_dim, **extra_kwargs).to(cfg.device)

        # load the weights into the target networks
        # self.successor_target_net.load_state_dict(self.successor_net.state_dict())
        # optimizers (mask all except feature_learner)
        self.encoder_opt: tp.Optional[torch.optim.Adam] = None
        # if cfg.obs_type == 'pixels':
        #     self.encoder_opt = torch.optim.Adam(self.encoder.parameters(), lr=cfg.lr)
        self.actor_opt: tp.Optional[torch.optim.Adam] = None
        self.sf_opt: tp.Optional[torch.optim.Adam] = None
        ...
        # # freeze all non-feature_learner parameters
        # for module in [self.encoder, self.actor, self.successor_net, self.successor_target_net]:
    ...
    def train(self, training: bool = True) -> None:
        ...
        if self.phi_opt is not None:
            if self.feature_learner is not None:
    ...
    def update_sf(
        ...
        # optimize: only update feature_learner
        if self.phi_opt is not None:
    ...
    def update(self, replay_loader: ReplayBuffer, step: int) -> tp.Dict[str, float]:
        ...
        for _ in range(self.cfg.num_sf_updates):
            ...
            local_metrics = self.update_sf(obs=obs, action=action, discount=discount, next_obs=next_obs, future_obs=future_obs,step=step)
            metrics.update(local_metrics)
            wandb.log(local_metrics)
        return metrics

    # New: dataset-driven update using HilbertRepresentationDataset batches
    def update_hilbert_batch(self, batch: tp.Mapping[str, torch.Tensor], step: int) -> tp.Dict[str, float]:
        metrics: tp.Dict[str, float] = {}
        obs = batch['obs'].to(self.cfg.device)
        next_obs = batch['next_obs'].to(self.cfg.device)
        future_obs = batch['goal_obs'].to(self.cfg.device)

        # encode/augment consistently with replay path
        ...
        if self.cfg.feature_learner == 'hilp':
            phi_loss, info = self.feature_learner(obs=obs, action=torch.zeros(obs.size(0), self.action_dim, device=obs.device), next_obs=next_obs, future_obs=future_obs)
        else:
            phi_loss = self.feature_learner(obs=obs, action=torch.zeros(obs.size(0), self.action_dim, device=obs.device), next_obs=next_obs, future_obs=future_obs)
            info = None
