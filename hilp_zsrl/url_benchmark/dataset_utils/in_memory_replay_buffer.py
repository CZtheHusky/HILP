...
from url_benchmark.dmc_utils.dmc import ExtendedTimeStep
    ...
    def get_episode(self, ep_idx=None, custom_reward=None):
        if ep_idx is None:
            if self._is_fixed_episode_length:
                ep_idx = np.random.randint(0, len(self))
            else:
                ...
                ep_idx = np.random.choice(np.arange(len(self._episodes_length)), p=self._episodes_selection_probability)
        episode_traj = self._storage['observation'][ep_idx]
        if custom_reward is not None:
            phy = self._storage["physics"][ep_idx]
            ds_rewards = custom_reward.from_physics(phy)
            ds_rewards = np.array(ds_rewards, dtype=np.float32)
        else:
            ds_rewards = self._storage['reward'][ep_idx]
