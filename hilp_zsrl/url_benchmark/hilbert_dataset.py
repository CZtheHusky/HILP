...
class HilbertRepresentationDataset(Dataset):
    def __init__(self, data_dir: str,
        ...
        for t in want_types:
            ...
            buf = DataBuffer.create_from_path(zpath, mode="r")
            new_buffer_dict = {
                "proprio": buf["proprio"][:] if use_history_action else buf["proprio"][..., :-19],
            }
            if self._full_loading:
                new_buffer_dict['actions'] = buf['actions'][:]
                new_buffer_dict['rewards'] = buf['rewards'][:]
                new_buffer_dict['privileged_obs'] = buf['critic_obs'][..., 76:79]
                new_buffer_dict['commands'] = buf['commands'][:, -1, :]
            new_buffer_dict["episode_ends"] = buf.episode_ends[:]
            self._buffers.append((t, new_buffer_dict))
    ...
    def __getitem__(self, idx):
        ...
        if self._full_loading:
            out['actions'] = torch.as_tensor(buf['actions'][t], dtype=torch.float32)
            out['rewards'] = torch.as_tensor(buf['rewards'][t], dtype=torch.float32)
            out['privileged_obs'] = torch.as_tensor(buf['privileged_obs'][t], dtype=torch.float32)
            out['commands'] = torch.as_tensor(buf['commands'][t], dtype=torch.float32)
        return out
